  ...
  def computeMD5Hash(args : Any*) : Array[Byte] = {
    ...
    def computeMD5HashR(a : Any) : Unit = {
      a match {
        ...
        case _ => {
          println("Can't convert: " + a.getClass().toString())
          throw new RuntimeException("Should not get here; Missing case!" +  a.getClass().toString())
        }
      }
    }
  ...
  def computeHash(args : Any*) : Int = {
    ...
    def computeHashR(acc : Int, a : Any) : Int = {
      a match {
        ...
        case any : Any => { println("test" + any.toString()); mix(acc, any.hashCode()) }
      }
    }
