...
class ExpandQuantifiers(expandMax: Int = 2048) extends Pass {
  ...
  private def expand(signal: mc.Signal): Iterable[mc.Signal] = {
    ...
    if(others.isEmpty) { List(signal) } else {
      others :+ signal.copy(e = e)
    }
  }

  private def expand(expr: smt.SMTExpr, prefix: String, bindings: Map[String, smt.BVExpr], others: mutable.ArrayBuffer[mc.Signal]): smt.SMTExpr= expr match {
